...
class PfEmail extends AbstractIntegration
{
        ...
        foreach (self::$branches as $branch) {
            try {
                \Bitrix\Main\Diag\Debug::writeToFile(
                    "Fetching PF email leads [{$branch}] from {$createdAtFrom} to {$createdAtTo}",
                    'PfEmail Start ' . date('Y-m-d H:i:s'),
                    $logFile
                );
                ...
                if ($status !== 200) {
                    \Bitrix\Main\Diag\Debug::writeToFile(
                        ...
                        "PfEmail API Error [{$branch}] " . date('Y-m-d H:i:s'),
                        $logFile
                    );
                    continue;
                }
                ...
                \Bitrix\Main\Diag\Debug::writeToFile(
                    $data,
                    "PfEmail Response [{$branch}] " . date('Y-m-d H:i:s'),
                    $logFile
                );

                if (empty($leads)) {
                    \Bitrix\Main\Diag\Debug::writeToFile(
                        "No email leads found [{$branch}]",
                        'PfEmail ' . date('Y-m-d H:i:s'),
                        $logFile
                    );
                    continue;
                }
                ...
                foreach ($leads as $lead) {
                    try {
                        ...
                        \Bitrix\Main\Diag\Debug::writeToFile(
                            ...
                            "PfEmail Deal Create Error [{$branch}] " . date('Y-m-d H:i:s'),
                            $logFile
                        );
                    }
                }
                ...
                \Bitrix\Main\Diag\Debug::writeToFile(
                    $e->getMessage(),
                    "PfEmail Branch Error [{$branch}] " . date('Y-m-d H:i:s'),
                    $logFile
                );
            }
        }
    ...
    private function processLead(array $lead, string $branch, string $logFile): void
    {
        ...
        $this->title          = 'PfEmail_' . $this->name . '_' . $this->email;
        ...
        \Bitrix\Main\Diag\Debug::writeToFile(
            ...
            'PfEmail Processing Lead ' . date('Y-m-d H:i:s'),
            $logFile
        );

        $this->createDeal('pf_email');
    }
}
